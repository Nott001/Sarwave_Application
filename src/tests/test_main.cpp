...
class ProtocolDefinitionsTest : public QObject {
    ...
    private slots:
        ...
        {
            QString json =
                R"({"x":1.5,"y":2.3,"z":0.8,"dopplerVelocity":0.5,"pointDensity":100,"snr":12.5,"spatialSpread":3.2,"distance":5.0,"classificationConfidence":0.95,"presenceDetected":true,"timestamp":"2026-01-01T00:00:00"})";
            RadarDetection data = parseRadarData(json);
            QCOMPARE(data.x, 1.5);
            QCOMPARE(data.y, 2.3);
            QCOMPARE(data.z, 0.8);
            QCOMPARE(data.dopplerVelocity, 0.5);
            QCOMPARE(data.pointDensity, 100.0);
            QCOMPARE(data.snr, 12.5);
            QCOMPARE(data.spatialSpread, 3.2);
            QCOMPARE(data.distance, 5.0);
            QCOMPARE(data.classificationConfidence, 0.95);
        ...
        void testSerializeRoundTrip()
        {
            ...
            original.y = 2.3;
            original.z = 0.8;
            original.dopplerVelocity = 0.5;
            original.pointDensity = 100.0;
            original.snr = 12.5;
            original.spatialSpread = 3.2;
            original.distance = 5.0;
            original.classificationConfidence = 0.95;
            ...
            QCOMPARE(parsed.pointDensity, original.pointDensity);
            QCOMPARE(parsed.snr, original.snr);
            QCOMPARE(parsed.spatialSpread, original.spatialSpread);
            QCOMPARE(parsed.distance, original.distance);
...
class GeneralTest : public QObject {
    ...
    private slots:
        ...
        {
            QVERIFY_EXCEPTION_THROWN(MMWave::Prompting::General::checkStrArgument("-1"),
                                     std::out_of_range);
        }

        void testCheckStrArgument_nonNumeric_throws()
        {
            QVERIFY_EXCEPTION_THROWN(MMWave::Prompting::General::checkStrArgument("abc"),
...
#ifdef _WIN32
            QCOMPARE(paths.cli, "COM1");
            QCOMPARE(paths.data, "COM2");
#else
            QCOMPARE(paths.cli, "/dev/ttyUSB1");
            QCOMPARE(paths.data, "/dev/ttyUSB2");
#endif
        }
};
...
class TlvTest : public QObject {
    ...
    private slots:
        ...
        void testTlvParsing()
        {
            ...
            bytes.insert(bytes.end(), std::begin(MMWave::Streaming::MAGIC_WORD),
                         std::end(MMWave::Streaming::MAGIC_WORD));
            uint8_t hb[32];
