    ...
    while (true) {
        try {
            ...
            for (const auto& tlv : MMWave::TlvRange(frame)) {
                if (tlv.type != MMWave::Configured::OutOfBox::TLV_DETECTED_POINTS) continue;

                for (const auto& [x, y, z, velocity] :
                    MMWave::Configured::OutOfBox::DetectedPoint::range(tlv)) {
                    std::println("  x={:.3f} y={:.3f} z={:.3f} v={:.3f}", x, y, z,
                                 velocity);
                }
            }
        }
