...
General::MMWavePortPaths portPathsFrom(QQuickWindow& window)
{
    ...
    const std::string commonName = window.property("commonName").toString().toStdString();
    return {
...
MMWave::Porter::Context portGUI()
{
    ...
    if (component.isError()) {
        throw std::runtime_error("Failed to load PortDialog.qml: " +
        ...
        try {
            ...
            if (MMWave::Porter::validateCliPort(*trial)) {
                ...
                if (firstNonWs == std::string::npos) {
                    ...
                    version = version.substr(firstNonWs, lastNonWs - firstNonWs + 1);
                }
                ...
                window->setProperty(
                    "errorMessage",
                    QStringLiteral("CLI port is not responding to an mmWave device. "
                                   "Check the port selection and try again."));
            }
        }
        catch (const boost::wrapexcept<boost::system::system_error>& e) {
            window->setProperty("errorMessage", QStringLiteral("Serial port does not exist: ") +
