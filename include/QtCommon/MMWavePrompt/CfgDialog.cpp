...
void appendCliReport(std::string& output, const std::string& line, const std::string& response)
{
    output += "-> " + line + "\n";
...
void streamCliReport(const std::string& line, const std::string& response)
{
    ...
    g_reportSink.window->setProperty("outputText", QString::fromStdString(*g_reportSink.output));
...
std::string runCfgDialog(MMWave::Porter::Context* ctx, const std::string& prefillPath)
{
    if (!qGuiApp) {
        throw std::runtime_error("CfgDialog requires a QGuiApplication instance");
    }
    ...
    OkHandler okHandler([&]() {
        try {
            ...
            if (path.empty()) {
                window->setProperty("errorMessage",
...
std::string cfgGUIAndSend(MMWave::Porter::Context& ctx, const std::string& path)
{
